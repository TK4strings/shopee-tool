...
const genOrderId = () => `SHP-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 9000) + 1000)}`;
 ...
 * 【規約遵守の設計方針】
 * - スクレイピングは一切行わない
 * - 公式API提供プラットフォームのみ自動同期
 * - レートリミッターで最短5分間隔を強制
 * 本番: Supabase Edge Function `/functions/v1/stock-sync` に置き換え
 */
...
// ============================================================
/**
 * 【Shopee Open Platform API 設計方針】
 *
 * 本番: フロントエンド → Supabase Edge Function → Shopee API
 *   partner_id / partner_key は Edge Function 環境変数で管理（フロント露出禁止）
 *
 * 出品取り下げAPI:
 *   POST /api/v2/product/unlist_item   → 非公開化（preferred）
 *   POST /api/v2/product/delete_item   → 完全削除（復元不可のため非推奨）
 *
 * 【禁止事項】
 *   × スクレイピング / ヘッドレスブラウザ操作
 *   × 非公式エンドポイント
 *
 * デモでは: CSV出力 + 手動対応ガイド を代替として提供
 */
const shopeeListingService = {
  async listProduct(product) {
    // MOCK: 本番 → Edge Function `/functions/v1/shopee-list` に置き換え
  ...
  async unlistProduct(product) {
    /**
     * 本番実装イメージ（Edge Function側）:
     *   POST /api/v2/product/unlist_item
     *   body: { item_id_list: [product.shopeeItemId], unlist: true }
     *
     * unlist: true  → 非公開化（在庫補充後に再公開可能）← 推奨
     * unlist: false → 再公開
     * delete_item   → 完全削除（要注意・復元不可）
     */
...
// § INITIAL DATA
// ============================================================
const P1 = nanoid(), P2 = nanoid(), P3 = nanoid();

/**
 * Product 型定義（TypeScript化時の参照）
 * @typedef {{
 *   id: string, name: string, sku: string, sourceUrl: string,
 *   sourcePlatform: string, sourceCost: number, shopeePrice: number,
 *   stock: number, previousStock: number, lastSyncedStock: number,
 *   stockChangeType: "up"|"down"|"same",
 *   status: "active"|"inactive"|"out_of_stock",
 *   isListed: boolean,
 *   listingStatus: "unlisted"|"active"|"paused"|"removed"|"error",
 *   unlistRequired: boolean,
 *   shopeeItemId?: string,
 *   listingLogs: Array<{id:string,at:string,action:string,message:string}>,
 *   category: string, extraCost: number, notes: string, createdAt: string
 * }} Product
 */
const mkProduct = (overrides) => ({
  ...
  isListed: false,
  listingStatus: "unlisted",   // "unlisted"|"active"|"paused"|"removed"|"error"
  unlistRequired: overrides.stock === 0 && false, // 初期は常にfalse
...
const INITIAL_PRODUCTS = [
  mkProduct({ id: P1, name: "ワイヤレスイヤホン Pro", sku: "WEP-001", sourceUrl: "https://aliexpress.com/item/001", sourcePlatform: "AliExpress", sourceCost: 1200, shopeePrice: 2980, stock: 50, status: "active", category: "電子機器", isListed: true, listingStatus: "active", shopeeItemId: "SHOPEE-DEMO0001" }),
  mkProduct({ id: P2, name: "シリコンスマホケース", sku: "SSC-002", sourceUrl: "https://taobao.com/item/002", sourcePlatform: "タオバオ", sourceCost: 300, shopeePrice: 980, stock: 200, status: "active", category: "スマホアクセサリー", isListed: false, listingStatus: "unlisted" }),
  mkProduct({ id: P3, name: "LEDデスクライト", sku: "LDL-003", sourceUrl: "https://amazon.co.jp/dp/003", sourcePlatform: "Amazon", sourceCost: 2200, shopeePrice: 4500, stock: 0, status: "out_of_stock", category: "家電", isListed: true, listingStatus: "active", shopeeItemId: "SHOPEE-DEMO0002", extraCost: 100, notes: "再入荷待ち", unlistRequired: true }),
];

const INITIAL_ORDERS = [
  { id: genOrderId(), productId: P1, productName: "ワイヤレスイヤホン Pro", buyer: "田中 太郎", quantity: 1, salePrice: 2980, sourceCost: 1200, shippingCost: 200, shopeeFeeRate: 0.03, extraCost: 0, status: "pending", orderDate: today(), trackingNo: "", notes: "", history: [{ at: today(), status: "pending", note: "注文登録" }] },
  { id: genOrderId(), productId: P2, productName: "シリコンスマホケース", buyer: "鈴木 花子", quantity: 2, salePrice: 1960, sourceCost: 600, shippingCost: 150, shopeeFeeRate: 0.03, extraCost: 0, status: "shipped", orderDate: today(), trackingNo: "TRK123456789", notes: "", history: [] },
  { id: genOrderId(), productId: P3, productName: "LEDデスクライト", buyer: "山田 一郎", quantity: 1, salePrice: 4500, sourceCost: 2200, shippingCost: 300, shopeeFeeRate: 0.03, extraCost: 0, status: "delivered", orderDate: today(), trackingNo: "TRK987654321", notes: "", history: [] },
];
...
const DEFAULT_SETTINGS = {
  ...
  syncEnabled: false,
  // 取り下げ設定
  autoUnlist: true,    // 在庫0検知時に自動取り下げを試みる
  unlistDelayMin: 0,       // 取り下げまでの猶予時間（分）。0=即時
};
...
const LISTING_COLORS = {
  unlisted: ["#6B7280", "#F3F4F6"],
  active: ["#10B981", "#ECFDF5"],
  paused: ["#F59E0B", "#FFFBEB"],
...
function reducer(state, action) {
  switch (action.type) {
    ...
    // ── Products ──────────────────────────────────────────
    case "ADD_PRODUCT":
      return { ...state, products: [...state.products, action.p] };
    case "UPDATE_PRODUCT":
      return { ...state, products: state.products.map(p => p.id === action.p.id ? { ...p, ...action.p } : p) };
    case "DELETE_PRODUCT":
      ...
      return {
        ...
        products: state.products.map(p => {
          ...
          return {
            ...p,
            listingStatus,
            isListed: isListed ?? p.isListed,
            unlistRequired: unlistRequired ?? false,
            shopeeItemId: shopeeItemId ?? p.shopeeItemId,
            listingLogs: [logEntry, ...p.listingLogs].slice(0, 50),
          };
        }),
      };
    }

    // ── unlistRequired を手動クリア ──────────────────────
    case "CLEAR_UNLIST_FLAG":
      return {
        ...state,
        products: state.products.map(p =>
          p.id === action.id ? { ...p, unlistRequired: false } : p
        ),
      };
    ...
    case "SHOW_CONFIRM": return { ...state, confirm: { message: action.message, onOk: action.onOk } };
...
function AppProvider({ children }) {
  ...
  // マウント時にSupabaseからデータを取得
  useEffect(() => {
    // 商品取得
    supabase.from("products").select("*")
      .order("created_at", { ascending: false })
      .then(({ data, error }) => {
        if (error) { console.error("商品取得エラー:", error); return; }
        if (!data) return;
    ...
    supabase.from("orders").select("*")
      .order("created_at", { ascending: false })
      .then(({ data, error }) => {
        if (error) { console.error("注文取得エラー:", error); return; }
        if (!data) return;
...
function useSyncScheduler() {
  ...
  const runSync = useCallback(async () => {
    ...
    try {
      ...
      // 在庫0かつ出品中の商品を検出 → 設定に応じて取り下げ処理
      const settings = settingsRef.current;
      const products = productsRef.current;
      const targets = updates.filter(u => u.newStock === 0).map(u => products.find(p => p.id === u.id)).filter(p => p && p.isListed && p.listingStatus === "active");

      if (targets.length > 0 && settings.autoUnlist) {
        targets.forEach(p => {
          if (unlistTimers.current[p.id]) return; // すでにスケジュール済み
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 10001, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 540, boxShadow: "0 8px 40px rgba(0,0,0,0.2)", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ background: "#FFFBEB", border: "1px solid #FCD34D", borderRadius: 10, padding: "10px 14px", marginBottom: 18, fontSize: 12, color: "#78350F", lineHeight: 1.6 }}>
          ...
          Shopee Open Platform APIはサーバーサイドのOAuth認証が必須のため、デモではフロントから直接呼び出せません。
          本番環境ではSupabase Edge Functionを経由して実装します。
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => downloadCsv(csv, `shopee_${product.sku}_${today()}.csv`)} style={{ ...S.btnPri, flex: 1 }}>📥 CSV出力（Shopeeインポート用）</button>
...
function UnlistGuideModal() {
  ...
  const steps = [
    ...
    ["2", "商品管理を開く", "サイドバーの「商品管理」>「商品一覧」を選択します。"],
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 10001, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 560, boxShadow: "0 8px 40px rgba(0,0,0,0.2)", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ background: "#FFFBEB", border: "1px solid #FCD34D", borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "#78350F", lineHeight: 1.6 }}>
          <strong>⚖️ 自動取り下げについて</strong><br />
          本番環境では Shopee API <code>unlist_item</code> をEdge Function経由で自動実行します。
          デモ環境では手動操作をご案内します。非公開化は復元可能のため「削除」より安全です。
        </div>
...
// § ORDER FORM
// ============================================================
const EMPTY_O = { productId: null, buyer: "", quantity: 1, salePrice: "", sourceCost: "", shippingCost: 0, shopeeFeeRate: 0.03, extraCost: 0, status: "pending", orderDate: today(), trackingNo: "", notes: "" };
        ...
        <div style={{ background: "#FEF2F2", border: "2px solid #EF4444", borderRadius: 12, padding: "14px 18px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#666", marginTop: 2 }}>
              {products.filter(p => p.unlistRequired).map(p => p.name).join("、")}
            </div>
          </div>
          <button onClick={() => setTab("products")} style={{ ...S.btnPri, background: "#EF4444", fontSize: 12, padding: "7px 14px", flexShrink: 0 }}>
            商品管理へ →
          </button>
        </div>
      )}
...
function ProductsTab() {
  ...
  const filtered = searched
    .filter(p => sfilt === "all" || p.status === sfilt)
    .filter(p => lsfilt === "all" || p.listingStatus === lsfilt);

  // 取り下げ対象（unlistRequired=true）
  ...
  const handleSave = async (data) => {
    if (editing) {
      ...
      const { error } = await supabase.from("products").insert({
        ...
        shopee_price: data.shopeePrice,
        stock: data.stock,
        previous_stock: data.stock,
        last_synced_stock: data.stock,
        stock_change_type: "same",
        status: data.status,
        is_listed: false,
        listing_status: "unlisted",
        unlist_required: false,
        category: data.category,
        extra_cost: data.extraCost,
        notes: data.notes,
      });
  ...
  const confirmUnlist = (p) =>
    dispatch({
      type: "SHOW_CONFIRM",
      message: `「${p.name}」をShopeeで非公開にしますか？\n在庫補充後に再公開できます。`,
      okLabel: "取り下げる", okColor: "#F59E0B",
      onOk: () => runUnlist(p),
    });

  // 出品ガイド表示（デモ）/ 本番では runListing(p) を直接呼ぶ
  const handleList = (p) =>
    dispatch({ type: "SHOW_MODAL", modal: { type: "guide", product: p } });

  const handleCsvExport = () => {
    const csv = shopeeListingService.generateCsv(filtered);
    downloadCsv(csv, `shopee_export_${today()}.csv`);
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {unlistTargets.length > 0 && (
            <button
              onClick={() => dispatch({ type: "SHOW_MODAL", modal: { type: "unlistGuide", products: unlistTargets } })}
      ...
      {unlistTargets.length > 0 && (
        <div style={{ background: "#FEF2F2", border: "1px solid #FCA5A5", borderRadius: 12, padding: "12px 16px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 13, color: "#DC2626" }}>
            <strong>🔕 在庫切れ出品商品があります:</strong>
            <span style={{ color: "#666", marginLeft: 8 }}>{unlistTargets.map(p => p.name).join("、")}</span>
          </div>
          <button
            onClick={() => dispatch({ type: "SHOW_MODAL", modal: { type: "unlistGuide", products: unlistTargets } })}
            style={{ ...S.btnDng, whiteSpace: "nowrap", marginLeft: 12 }}>
            手動対応ガイド
          </button>
        </div>
      )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
            {filtered.map(p => {
              ...
              const canList = !p.isListed && !["removed"].includes(p.listingStatus);
                ...
                <tr key={p.id} style={{ borderBottom: "1px solid #F3F4F6", background: rowBg }}>
                  <td style={{ padding: "12px 14px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      {p.unlistRequired && <span title="在庫切れ・取り下げ必要" style={{ fontSize: 14 }}>🔕</span>}
                    </div>
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    ...
                    {p.stockChangeType !== "same" && (
                      <div style={{ fontSize: 10, color: STOCK_CHANGE_COLOR[p.stockChangeType] }}>前回: {p.previousStock}</div>
                    )}
                    {p.stock === 0 && p.isListed && (
                      <div style={{ fontSize: 10, color: "#EF4444", fontWeight: 700 }}>⚠ 出品中</div>
                    )}
                  </td>
                  {/* 出品ステータス */}
                  <td style={{ padding: "12px 14px" }}>
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {/* 出品ボタン */}
                      {canList && (
                        <button onClick={() => !isProcessing && handleList(p)}
                          style={isProcessing ? S.btnListDis : S.btnList} disabled={isProcessing}>
                          {isProcessing ? "⏳" : "🏪 出品"}
                        </button>
                      )}
                      {/* 取り下げボタン */}
                      {canUnlist && (
                        <button onClick={() => !isProcessing && confirmUnlist(p)}
                          style={isProcessing ? S.btnListDis : S.btnUnlist} disabled={isProcessing}>
                          {isProcessing ? "⏳" : "🔕 取り下げ"}
                        </button>
                      )}
                      {/* 出品ログ */}
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} style={{ textAlign: "center", padding: 40, color: "#aaa" }}>商品がありません</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div style={{ display: "grid", gap: 20 }}>
        ...
        <div style={S.card}>
          ...
          <p style={{ margin: "0 0 18px", fontSize: 12, color: "#888" }}>
            在庫同期で stock=0 を検知した出品商品を取り下げます。
            本番API環境では Shopee API <code>unlist_item</code> を実行。デモは手動ガイドを表示します。
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
            ...
            <div>
              ...
              <select style={{ ...S.inp, opacity: settings.autoUnlist ? 1 : 0.5 }} value={settings.unlistDelayMin} onChange={e => setSetting("unlistDelayMin", +e.target.value)}
                disabled={!settings.autoUnlist} >
        ...
        <div style={{ background: "#FFFBEB", border: "1px solid #FCD34D", borderRadius: 12, padding: "14px 18px" }}>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 12, color: "#78350F", lineHeight: 1.7 }}>
            ...
            ・自動取り下げ: 猶予時間設定で誤検知による意図しない取り下げを防止<br />
...
export default function App() {
  ...
  const appDispatchRef = useRef(null);
        ...
        data.forEach(row => {
          appDispatchRef.current({
            ...
            p: {
              ...
              lastSyncedStock: row.last_synced_stock,
              stockChangeType: row.stock_change_type || "same",
              status: row.status,
              isListed: row.is_listed,
              listingStatus: row.listing_status,
              unlistRequired: row.unlist_required,
              shopeeItemId: row.shopee_item_id,
              category: row.category || "",
        ...
        data.forEach(row => {
          appDispatchRef.current({
            ...
            o: {
              ...
              shopeeFeeRate: row.shopee_fee_rate,
              extraCost: row.extra_cost,
              status: row.status,
  ...
  return (
    <AppProvider dispatchRef={appDispatchRef}>
      <ToastProvider>
        <AppInner />
      </ToastProvider>
    </AppProvider>
